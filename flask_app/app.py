...
def analytics_payload() -> dict[str, Any]:
    roles = fetch_all("SELECT role, COUNT(*) AS total FROM users GROUP BY role ORDER BY role")
    roles_map = {row["role"]: int(row["total"]) for row in roles}

    cases = fetch_all("SELECT status, COUNT(*) AS total FROM cases GROUP BY status ORDER BY status")
    cases_map = {row["status"]: int(row["total"]) for row in cases}

    priorities = fetch_all("SELECT priority, COUNT(*) AS total FROM cases GROUP BY priority ORDER BY priority")
    ...
    recent_messages = fetch_all(
        """
        SELECT m.sent_at, m.is_read, su.full_name AS sender_name, su.role AS sender_role,
               ru.full_name AS receiver_name, ru.role AS receiver_role, c.case_number
        FROM messages m
    ...
    top_lawyers = fetch_all(
        """
        SELECT cu.full_name AS lawyer_name, COUNT(*) AS total
        FROM cases c
        JOIN lawyers l ON l.id = c.lawyer_id
    ...
    top_clients = fetch_all(
        """
        SELECT cu.full_name AS client_name, COUNT(*) AS total
        FROM cases c
        JOIN clients cl ON cl.id = c.client_id
        JOIN users cu ON cu.id = cl.user_id
        GROUP BY cu.id, cu.full_name
...
def dashboard():
    ...
    <body>
      <main class="shell">
        ...
        <section class="two-col">
          <article class="card">
            <h2 style="margin-top:0;">Top Lawyers</h2>
            <div class="list">
              {"".join(
                f"""
                <div class="item">
