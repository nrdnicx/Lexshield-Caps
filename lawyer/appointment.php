          ...
          <?php foreach ($appointments as $appointment): ?>
            <tr class="appointment-row">
              <td data-label="Case / Client">
                <div class="appointment-cell-title"><?= lex_e(($appointment['case_number'] ?? '') . ' - ' . ($appointment['title'] ?? 'Appointment')) ?></div>
                <div class="appointment-cell-subtitle"><?= lex_e((string) ($appointment['client_name'] ?? '')) ?></div>
                <?php if (!empty($appointment['notes'])): ?>
                  <div class="appointment-cell-notes"><?= lex_e((string) $appointment['notes']) ?></div>
                <?php endif; ?>
              </td>
              <td data-label="Scheduled"><?= lex_e(date('M j, Y g:i A', strtotime((string) $appointment['scheduled_at']))) ?></td>
              ...
              <td data-label="Actions">
                <div class="appointment-actions">
                  ...
                  <form method="post" action="<?= lex_e($currentUrl) ?>" class="appointment-inline-form" data-no-loading>
                    ...
                    <input type="hidden" name="appointment_id" value="<?= (int) $appointment['id'] ?>">
                    <button class="button button-danger appointment-action-button" type="submit" aria-label="Delete appointment" title="Delete appointment" data-confirm="Delete this appointment request?">&times;</button>
                  </form>
                </div>
              </td>
            </tr>
...
{
    if ($totalAll <= 0 || $totalPages <= 1) {
        return '';
    }

    $links = lex_appointment_render_page_links($currentPage, $totalPages, $state);
    ob_start();
    ?>
    <nav class="appointment-pagination" aria-label="Appointment pagination">
        <div class="appointment-pagination-summary">
        Showing <?= (int) (($currentPage - 1) * 10 + 1) ?>-<?= (int) min($totalFiltered, $currentPage * 10) ?> of <?= (int) $totalFiltered ?>
      </div>
      <div class="pagination-bar">
        ...
        ?>
        <a class="pagination-button<?= $currentPage <= 1 ? ' is-disabled' : '' ?>" href="<?= lex_e(lex_appointment_state_url($prevState)) ?>" data-appointment-page data-page="<?= max(1, $currentPage - 1) ?>"<?= $currentPage <= 1 ? ' aria-disabled="true" tabindex="-1"' : '' ?>>Previous</a>

        <?php foreach ($links as $link): ?>
          <?php if ($link === 'ellipsis-start' || $link === 'ellipsis-end'): ?>
            <span class="pagination-ellipsis" aria-hidden="true">&hellip;</span>
          <?php else: ?>
            <?php
            $pageState = $state;
            $pageState['page'] = (int) $link;
            ?>
            <a class="pagination-button<?= $currentPage === (int) $link ? ' is-active' : '' ?>" href="<?= lex_e(lex_appointment_state_url($pageState)) ?>" data-appointment-page data-page="<?= (int) $link ?>"<?= $currentPage === (int) $link ? ' aria-current="page"' : '' ?>><?= (int) $link ?></a>
          <?php endif; ?>
        <?php endforeach; ?>
        ...
        ?>
        <a class="pagination-button<?= $currentPage >= $totalPages ? ' is-disabled' : '' ?>" href="<?= lex_e(lex_appointment_state_url($nextState)) ?>" data-appointment-page data-page="<?= min($totalPages, $currentPage + 1) ?>"<?= $currentPage >= $totalPages ? ' aria-disabled="true" tabindex="-1"' : '' ?>>Next</a>
      </div>
    </nav>
    <?php
    return (string) ob_get_clean();
...
if ($state['q'] !== '') {
    ...
    $searchWhere = ' AND LOWER(CONCAT_WS(" ", c.case_number, c.title, u.full_name, u.email, a.status, COALESCE(a.notes, ""), DATE_FORMAT(a.scheduled_at, "%M %e, %Y %l:%i %p"), DATE_FORMAT(a.scheduled_at, "%Y-%m-%d %H:%i"), a.id)) LIKE :search ';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && lex_csrf_validate($_POST['csrf_token'] ?? null)) {
    $postState = lex_appointment_state_from_request($_POST);
...
$stmt = $pdo->prepare(
    'SELECT a.id, a.case_id, a.scheduled_at, a.status, a.notes, c.case_number, c.title, u.full_name AS client_name
     FROM appointments a
     JOIN cases c ON c.id = a.case_id
     JOIN clients cl ON cl.id = c.client_id
     JOIN users u ON u.id = cl.user_id'
...
<section class="card appointment-card" data-appointment-board data-endpoint="<?= lex_e(lex_app_url('lawyer/appointment.php')) ?>">
  <div class="card-head appointment-head">
    <div>
      ...
      <p class="muted" data-appointment-summary><?= lex_e($summaryText) ?></p>
    </div>
  ...
  <?php if ($showFilters): ?>
    <form method="get" action="<?= lex_e($returnUrl) ?>" class="appointment-filters" data-appointment-filters data-no-loading>
      ...
      <button class="button button-secondary appointment-search-submit" type="submit">Search</button>
    </form>
  <?php endif; ?>
